...
fn dec(m: &mut Machine, mem: &mut Memory, addr: u16) {
    let result = mem.get8(addr).wrapping_sub(1);
    mem.set8(addr, result);
    update_zero_flag(m, result);
}

fn inc(m: &mut Machine, mem: &mut Memory, addr: u16) {
    let result = mem.get8(addr).wrapping_add(1);
    mem.set8(addr, result);
    update_zero_flag(m, result);
}
...
pub const INSTRUCTIONS: &[Instruction] = &[
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.clear_bit("SR", CARRY),
        test: |m, mem| {
            m.set_bit("SR", CARRY);
            m.run_program(
                mem,
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.clear_bit("SR", DECIMAL),
        test: |m, mem| {
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.clear_bit("SR", INTERRUPT),
        test: |m, mem| {
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.clear_bit("SR", OVERFLOW),
        test: |m, mem| {
            m.set_bit("SR", OVERFLOW);
            m.run_program(
                mem,
    ...
    Instruction {
        ...
        cycles: 3,
        execute: |m, mem| {
            let addr = m.fetch8(mem);
            let value = mem.get8(addr.into());
            compare(m, "AC", value);
        },
        test: |m, mem| {
            m.set_reg("AC", 0x05);
    ...
    Instruction {
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x10), 0x00, "wrong value");
            assert!(m.test_bit("SR", ZERO), "zero flag not set");
            ...
            assert_eq!(mem.get8(0x10), 0xFF, "wrong value");
            assert!(!m.test_bit("SR", ZERO), "zero flag not cleared");
        },
    },
    Instruction {
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x1000), 0x00, "wrong value");
            assert!(m.test_bit("SR", ZERO), "zero flag not set");
            ...
            assert_eq!(mem.get8(0x1000), 0xFF, "wrong value");
            assert!(!m.test_bit("SR", ZERO), "zero flag not cleared");
        },
    },
    Instruction {
    ...
    Instruction {
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x10), 0x01, "wrong value");
            assert!(!m.test_bit("SR", ZERO), "zero flag not cleared");
        },
    },
    Instruction {
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x1000), 0x01, "wrong value");
            assert!(!m.test_bit("SR", ZERO), "zero flag not cleared");
        },
    },
    Instruction {
    ...
    Instruction {
        ...
        cycles: 4,
        execute: |m, mem| {
            let base = m.fetch16(mem);
            let addr = base.wrapping_add(m.reg("XR").into());
            load_reg(m, "AC", mem.get8(addr));
        },
        test: |m, mem| {
            mem.set8(0x0102, 0xFF);
            m.run_program(
                mem,
                &[
                    0xA2, 0x02, //       $0000 LDX #$02
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.set_bit("SR", CARRY),
        test: |m, mem| {
            m.run_program(
                mem,
                &[
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.set_bit("SR", DECIMAL),
        test: |m, mem| {
            m.run_program(
                mem,
                &[
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.set_bit("SR", INTERRUPT),
        test: |m, mem| {
            m.run_program(
                mem,
                &[
    ...
    Instruction {
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x05), 0x42, "wrong value at address");
        },
    },
    Instruction {
        mnemonic: "STA",
        ...
        test: |m, mem| {
            ...
            assert_eq!(mem.get8(0x0006), 0x42, "wrong value at address");
        },
    },
    Instruction {
        mnemonic: "STX",
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| load_reg(m, "XR", m.reg("AC")),
        test: |m, mem| {
            m.set_reg("AC", 0x42);
            m.set_bit("SR", ZERO);
            m.run_program(
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| load_reg(m, "YR", m.reg("AC")),
        test: |m, mem| {
            m.set_reg("AC", 0x42);
            m.set_bit("SR", ZERO);
            m.run_program(
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| load_reg(m, "XR", m.reg("SP")),
        test: |m, mem| {
            m.set_reg("SP", 0x42);
            m.set_bit("SR", ZERO);
            m.run_program(
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| load_reg(m, "AC", m.reg("XR")),
        test: |m, mem| {
            m.set_reg("XR", 0x42);
            m.set_bit("SR", ZERO);
            m.run_program(
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| m.set_reg("SP", m.reg("XR")),
        test: |m, mem| {
            m.set_reg("XR", 0x42);
            m.run_program(
                mem,
    ...
    Instruction {
        ...
        bytes: 1,
        cycles: 2,
        execute: |m, _| load_reg(m, "AC", m.reg("YR")),
        test: |m, mem| {
            m.set_reg("YR", 0x42);
            m.set_bit("SR", ZERO);
            m.run_program(
