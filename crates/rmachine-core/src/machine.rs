...
impl Machine {
    ...
    /// # Errors
    ///
    /// Returns any exception error from [`step()`](`Self::step`).
    ...
    /// # Errors
    ///
    /// Returns any exception generated by the instruction (for example,
    /// “break”).
    ...
    /// Fetch the next 2 bytes from memory.
    ///
    /// # Errors
    ///
    /// Returns an error if the address is out of bounds.
...
mod tests {
    ...
    pub const INSTRUCTIONS: &[Instruction] = &[
        Instruction {
            ...
            bytes: 1,
            cycles: 2,
            execute: |m| m.exception = Some("break".into()),
            test: |m| {
                m.run_program(&[
                    0x00, // 0x0000 BRK
                ]);
        ...
        Instruction {
            ...
            test: |m| {
                ...
                assert_eq!(m.reg("A"), 0xFF, "wrong A");
            },
        },
        Instruction {
            mnemonic: "NOP",
